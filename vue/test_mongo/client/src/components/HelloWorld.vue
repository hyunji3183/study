    ...
    <div class="hello">
        ...
        <ul>
            <li v-for="item in data" :key="item.date">
                ...
                <button @click="update(item.date, item.count)"> like </button>
                <button @click="del(item.date)"> delete </button>
            </li>
        </ul>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        update(date, count) {
            axios.put(`http://localhost:3000/api/update?date=${date}&count=${count}`)
                .then(res => {
                    this.data = res.data;
                })
        }
